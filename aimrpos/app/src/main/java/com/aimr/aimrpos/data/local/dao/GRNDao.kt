...
interface GRNDao {
    ...
    @Query("SELECT * FROM grns WHERE locationId = :locationId AND isDeleted = 0")
    fun getByLocation(locationId: String): Flow<List<GRNEntity>>
    ...
    @Query("CREATE INDEX IF NOT EXISTS idx_grn_location ON grns(locationId)")

...
interface StockTransferDao {
    ...
    @Query("SELECT * FROM stock_transfers WHERE fromLocationId = :locationId AND isDeleted = 0")
    fun getByFromLocation(locationId: String): Flow<List<StockTransferEntity>>

    @Query("SELECT * FROM stock_transfers WHERE toLocationId = :locationId AND isDeleted = 0")
    fun getByToLocation(locationId: String): Flow<List<StockTransferEntity>>
    ...
    @Query("CREATE INDEX IF NOT EXISTS idx_stock_from ON stock_transfers(fromLocationId)")
    suspend fun indexFromLocation()

    @Query("CREATE INDEX IF NOT EXISTS idx_stock_to ON stock_transfers(toLocationId)")
    suspend fun indexToLocation()
}